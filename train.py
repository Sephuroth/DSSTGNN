...
parser.add_argument("--data", type=str, default="METEO", help="data path")
parser.add_argument("--input_dim", type=int, default=5, help="input_dim")
...
def main():
    ...
    for i in range(1, args.epochs + 1):
        ...
        for iter, (x, y) in enumerate(dataloader["train_loader"].get_iterator()):
            ...
            trainx = trainx.transpose(1, 3)   # (batch, times, num_nodes, n_features) --> (B, F, N, T)
            trainy = torch.Tensor(y).to(device)
            trainy = trainy.transpose(1, 3)
            metrics = engine.train(trainx, trainy[:, 0:3, :, :])   # 将traniy[:, 0, :, :]改成trainy
        ...
        for iter, (x, y) in enumerate(dataloader["val_loader"].get_iterator()):
            ...
            metrics = engine.eval(testx, testy[:, 0:3, :, :])
            valid_loss.append(metrics[0])
            valid_mape.append(metrics[1])
            valid_rmse.append(metrics[2])
            valid_wmape.append(metrics[3])

            A_graph, D_graph = metrics[4], metrics[5]  # 取出图
        ...
        if mvalid_loss < loss:
            ...
            if i < 100:
                ...
                np.save(os.path.join(graph_path, f"A_graph_epoch{i}.npy"),
                        A_graph.detach().cpu().numpy())
                np.save(os.path.join(graph_path, f"D_graph_epoch{i}.npy"),
                        D_graph.detach().cpu().numpy())
                ...
                for iter, (x, y) in enumerate(dataloader["test_loader"].get_iterator()):
                    ...
                    testx = testx.transpose(1, 3)
                    with torch.no_grad():
                        preds, A_graph, D_graph = engine.model(testx)
